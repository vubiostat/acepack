context("ACE Permutation Test (ace.test)")
...
{
  expect_no_error(x <- ace.test(d200))
...
test_that("Pulls variable names",
{
  joe <- d5[,1]
  jim <- d5[,2]
  expect_no_error(x <- ace.test(joe, jim))
...
test_that("Pulls matrix names",
{
  ...
  expect_no_error(x <- ace.test(zed))
...
{
  expect_no_error(x <- ace.test(as.data.frame(d5)))
...
{
  expect_error(ace.test(matrix(1:9, ncol=3)), 'must be 2 columns')
  expect_error(ace.test(matrix(1:9, ncol=1)), 'must be 2 columns')
})

test_that("Cannot have a matrix 'x' and a y specified",
{
  expect_error(ace.test(matrix(1:8, ncol=2), 1:10), 
    "Cannot have a matrix for 'x' and provide 'y'")
})
  
test_that("When x is not a matrix must have a y",
{
  expect_error(ace.test(1:4), "Must supply both 'x' and 'y'")
})

test_that("'nperm' must be a positive integer",
{
  expect_error(ace.test(1:4, 5:8, nperm=-1),  "'nperm' must be a positive integer")
  expect_error(ace.test(1:4, 5:8, nperm='r'), "'nperm' must be a positive integer")
  expect_error(ace.test(1:4, 5:8, nperm=2:3), "'nperm' must be a positive integer")
})

test_that("Length of 'x' and 'y' must be the same",
{
  expect_error(ace.test(1:2, 5:10), "Length of 'x' and 'y' must be the same")
})
